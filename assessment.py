...
def is_hometown(town):
    """Return True if input equals town.
...
def first_last_name(first, last):
    """Concatenate first and last name in one string.
...
def same_hometown(town, first, last):
    """Takes first and last name as input and runs the first_last_name function.

    Takes town as an argument and runs the is_hometown function printing script based on output.
    """
    hometown = is_hometown(town)
    full_name = first_last_name(first, last)
    # print hometown
    # print full_name
    if hometown is True:
        print "Hi", full_name, "we're from the same place!"
    else:
        print "I'd like to visit", town, "!"

same_hometown("Seattle", "Honey", "Wicks")
###############################################################################
...
def calculate_price(price, state, tax = .05):
    ...
    if state == "CA":
        total = price_with_tax + (price_with_tax * .03)
    elif state == "PA":
        total = price_with_tax + 2
    elif state == "MA":
        if price < 100:
            total = price_with_tax + 1
        else:
            total = price_with_tax + 3
    else:
...
def add_multiples_to_list(lst, *args):
    """Takes in a list of items and an arbitrary number of items.

    Then adds the arbitrary number of items to the list.
    """
...
print add_multiples_to_list(["apple", "berry", "cherry"], "hop", "skip", "jump")
